...
export default class Send extends Component<Props, State> {
  ...
  constructor(props?: Props) {
    ...
    this.handleAmountChangeFee = this.handleAmountChangeFee.bind(this);
  ...
  handleAmountChangeFee = (event: any) => {
    let enteredAmountFee = event.target.value;
    if (enteredAmountFee === '') {
      this.setState({
        enteredAmountFee: ''
      });
    ...
    const regex = /^\d*(\.(\d{1,8}?)?)?$/;
    if (!regex.test(enteredAmountFee) === true) {
      return;
    }
  ...
  prepareTransaction = (event: any) => {
    ...
    const transactionData = {
      ...
      fee: Number(enteredAmountFee) * (10 ** Config.decimalPlaces),
      paymentID,
      sendAll
    };
        ...
        <div className={`wholescreen ${backgroundColor}  hide-scrollbar`}>
          ...
          <div className={`maincontent ${backgroundColor} ${pageAnimationIn}`}>
            <form onSubmit={this.prepareTransaction}>
              ...
              <div className="field">
                <div className="control">
                  <label className={`label ${textColor}`} htmlFor="fee">
                    {i18n.fee_to_send}
                    <input
                      className="input is-large"
                      type="text"
                      placeholder={i18n.fee_to_send_placeholder}
                      value={enteredAmountFee}
                      onChange={this.handleAmountChangeFee}
                      id="fee"
                    />
                  </label>
                </div>
